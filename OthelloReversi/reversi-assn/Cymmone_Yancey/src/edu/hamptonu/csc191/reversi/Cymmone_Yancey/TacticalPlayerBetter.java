 ...
 */
public class TacticalPlayerBetter implements Player {
  private Color color;
  private GameState gameState;
  private Random rand;

  public TacticalPlayerBetter(Color color) {
    this.color = color;
  ...
  public List<Location> maxScores(Map<Integer, Location> input){
    List<Location> ret = new ArrayList<Location>();
    int max = Integer.MIN_VALUE;
    for(int score : input.keySet()){
      max = Math.max(score, max);
    }
    for(int score : input.keySet()){
      if(score == max){
        ret.add(input.get(score));
      }
    }
    return ret;
  }
 private boolean isFrontier(Location loc){
  ...
  private List<Location> minFrontier(List<Location> maxScores){
    List<Integer> runScores = new ArrayList<>();
    List<Location> minFrontScores = new ArrayList<>();
    for(Location loc : maxScores){
      int count = 0;
    ...
    for(int score : runScores){
      min = Math.min(score, min);
    }
    for(int score : runScores){
      if(score == min){
        minFrontScores.add(maxScores.get(runScores.indexOf(score)));
      }
    }
  ...
  private Location bestMove(List<Integer> aScores, List<Location> aLegalMoves){
    ...
    if(!cornerMoves.isEmpty()){
      List<Location> best = maxScores(cornerMoves);
      List<Location> front = minFrontier(best);
      return front.get(rand.nextInt(front.size()));
    }else if(!edgeMoves.isEmpty()){
      /*List<Location> best = maxScores(edgeMoves);
      return best.get(rand.nextInt(best.size()));*/
      List<Location> best = maxScores(edgeMoves);
      List<Location> front = minFrontier(best);
      return front.get(rand.nextInt(front.size()));
    }else if(!criticalMoves.isEmpty()){
      /*List<Location> best = maxScores(criticalMoves);
      return best.get(rand.nextInt(best.size()));*/
      List<Location> best = maxScores(criticalMoves);
      List<Location> front = minFrontier(best);
      return front.get(rand.nextInt(front.size()));
    } else if(!otherMoves.isEmpty()){
      /*List<Location> best = maxScores(otherMoves);
      return best.get(rand.nextInt(best.size()));*/
      List<Location> best = maxScores(otherMoves);
      List<Location> front = minFrontier(best);
      return front.get(rand.nextInt(front.size()));
    } else {
      /*List<Location> best = maxScores(lMoves);
      return best.get(rand.nextInt(best.size()));*/
      List<Location> best = maxScores(lMoves);
      List<Location> front = minFrontier(best);
      return front.get(rand.nextInt(front.size()));
  ...
  public Location nextMove(Instant deadline) {
    ...
    Location move = bestMove(scores, legalMoves);
